...
export const validateCollectionRequest = (body) => {
  ...
  if (!body.template) {
    ...
    if (
      body.vectors &&
      body.vectors.distance &&
      !['Cosine', 'Euclid', 'Dot'].includes(body.vectors.distance)
    ) {
  ...
  if (body.backup) {
    ...
    if (
      body.backup.retentionCount &&
      (body.backup.retentionCount < 1 || !Number.isInteger(body.backup.retentionCount))
    ) {
