...
export const executeRestore = async (restoreObj, k8sCustomApi, k8sBatchApi) => {
    ...
    try {
        ...
        if (!collection.spec.snapshots) {
            throw new Error('Collection does not have snapshot configuration. Cannot restore without S3 config.');
        }
...
export const updateRestoreStatus = async (restoreObj, phase, message = null, error = null) => {
    const status = {
        ...
        ...(message && { message }),
        ...(error && { error }),
        ...(phase === 'InProgress' && !restoreObj.status?.startedAt && {
            startedAt: new Date().toISOString()
        }),
        ...(phase === 'Completed' && !restoreObj.status?.completedAt && {
            completedAt: new Date().toISOString()
        }),
        ...(phase === 'Failed' && !restoreObj.status?.completedAt && {
            completedAt: new Date().toISOString()
        })
    };
