...
while [ $elapsed -lt $timeout ]; do
  if kubectl get cronjob ${CRONJOB_NAME} -n default >/dev/null 2>&1; then
    log_info "✅ VolumeSnapshot CronJob created: ${CRONJOB_NAME}"
...
if [ "${cronjob_created}" = "false" ]; then
  ...
  log_info "Waiting for first snapshot job to be created (may take up to 2 minutes)..."
  ...
  sleep 30  # Wait a bit more for snapshots to be created
